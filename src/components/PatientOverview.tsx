                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold">
                      {staff.find(u => u.id === selectedPatient.attending_doctor_id)?.name || (() => {
                        const doctors = staff.filter((u: any) => u.role?.toUpperCase() === 'DOCTOR' || u.role?.toUpperCase() === 'SUPER_ADMIN' || u.role?.toUpperCase() === 'SURGEON');
                        const defaultDoc = doctors.find((u: any) => u.role?.toUpperCase() === 'DOCTOR') || doctors.find((u: any) => u.name && u.name.toLowerCase().includes('dr.')) || doctors[0];
                        return defaultDoc ? defaultDoc.name : 'Dr. Rajesh Sharma';
                      })()}
                    </p>
