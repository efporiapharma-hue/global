...
export default function LISResultEntry({ onRelease }: { onRelease?: () => void }) {
  ...
  const evaluateValueStatus = (parameterId: string, strVal: string, gender: string, age: number): {
    ...
    if (isNaN(val)) {
      return { status: 'Normal', rangeStr: `${ref.lowRange} - ${ref.highRange}`, unit: ref.unit };
    }

    const rangeStr = `${ref.lowRange.toFixed(ref.lowRange % 1 === 0 ? 0 : 2)} - ${ref.highRange.toFixed(ref.highRange % 1 === 0 ? 0 : 2)}`;
    const unit = ref.unit;
    ...
    if (val < ref.lowRange) return { status: 'Low', rangeStr, unit };
    if (val > ref.highRange) return { status: 'High', rangeStr, unit };
    ...
    Object.keys(selectedRecord.results).forEach(pid => {
      ...
      if (parameterObj) {
        ...
        releasedResults[pid] = {
          ...
          referenceRangeStr: evaluation.rangeStr,
          status: evaluation.status,
          interpretation: evaluation.status === 'Normal' ? 'Normal Baseline' : `${parameterObj.name} is abnormal (${evaluation.status})`,
          isFormulaBased: parameterObj.formulaBased
        };
      }
    });
              ...
              {newOrder.patientId && (
                <div className="mt-2 p-2 bg-indigo-50 border border-indigo-100 rounded-lg flex items-center gap-2.5">
                  <div className="h-7 w-7 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-[10px] uppercase shrink-0">
                    {patients.find(pat => pat.id === newOrder.patientId)?.name.substring(0, 2)}
                  </div>
